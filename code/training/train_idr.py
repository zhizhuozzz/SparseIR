            ...
            for data_index, (indices, model_input, ground_truth) in enumerate(self.train_dataloader):
                ...
                if self.cur_iter % 50 == 0:
                    print('{0} [{1}] ({2}/{3}): loss = {4}, '
                        'idr_rgb_loss = {5}, eikonal_loss = {6}, mask_loss = {7}, normal_l1_loss = {8}, normal_cos_loss = {9}, idr_psnr = {10}, idr_lr = {11}'
                            .format(self.expname, epoch, data_index, self.n_batches, 
                                    loss.item(),
                                    loss_output['idr_rgb_loss'].item(),
                                    loss_output['eikonal_loss'].item(),
                                    loss_output['mask_loss'].item(),
                                    loss_output['normal_l1_loss'].item(),
                                    loss_output['normal_cos_loss'].item(),
                    ...
                    self.writer.add_scalar('normal_l1_loss', loss_output['normal_l1_loss'].item(), self.cur_iter)
                    self.writer.add_scalar('normal_cos_loss', loss_output['normal_cos_loss'].item(), self.cur_iter)
                self.cur_iter += 1
