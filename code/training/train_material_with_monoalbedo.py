...
class MaterialTrainRunner():
    def __init__(self,**kwargs):
        ...
        # albedo or albedo grad
        self.albedo_grad = self.conf.get_float('loss.albedo_grad_weight') > 0
        if self.albedo_grad:
            print('albedo_grad activated')
        if self.albedo_grad:
            self.expname = 'Mat-' + kwargs['expname'] + '_monoalbedo_grad_001'
        else:
            self.expname = 'Mat-' + kwargs['expname'] + '_monoalbedo'
    ...
    def plot_to_disk(self):
        ...
        if self.albedo_grad:
            self.train_dataset.change_sampling_idx_for_albedo_grad_supervision(-1)
        else:
    ...
    def run(self):
        ...
        for epoch in range(self.start_epoch, self.nepochs + 1):
            if self.albedo_grad:
                self.train_dataset.change_sampling_idx_for_albedo_grad_supervision(self.num_pixels)
            else:
                ...
                if self.cur_iter % 50 == 0:
                    if self.albedo_grad:
                        print('{0} [{1}] ({2}/{3}): loss = {4}, '
                            'sg_rgb_loss = {5}, sg_lr = {6}, sg_psnr = {7}, kl_loss={8}, latent_smooth_loss={9}, albedo_grad_loss={10}'
                                .format(self.expname, epoch, indices, self.n_batches, 
                                        ...
                                        loss_output['albedo_grad_loss'].item()))
                    else:
                        print('{0} [{1}] ({2}/{3}): loss = {4}, '
                            'sg_rgb_loss = {5}, sg_lr = {6}, sg_psnr = {7}, kl_loss={8}, latent_smooth_loss={9}, albedo_loss={10}'
                                .format(self.expname, epoch, indices, self.n_batches, 
                                        loss.item(),
                                        loss_output['sg_rgb_loss'].item(),
                                        self.sg_scheduler.get_last_lr()[0],
                    ...
                    if self.albedo_grad:
                        self.writer.add_scalar('albedo_grad_loss', loss_output['albedo_grad_loss'].item(), self.cur_iter)
                    else:
                        self.writer.add_scalar('albedo_loss', loss_output['albedo_loss'].item(), self.cur_iter)
